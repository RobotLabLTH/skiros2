...
class IndividualsDataset(Ontology):
    ...
    def get_individual(self, name, context_id=""):
        ...
        if not self.uri_exists(subject, context_id):
            raise Exception("Element {} doesn't exist in ontology. Uri: {}. Context: {}. Existing: {}".format(
                name, subject, context_id, [self.uri2lightstring(s) for s in self.context.subjects(RDF.type, OWL.NamedIndividual)]))
        e = Element()
